...
void _init(void)
{
    ringbuffer_init(&rx_buf, rx_mem, 1024);
    ringbuffer_init(&tx_buf, tx_mem, 1024);
    uart_init(UART_0, 115200, rx_cb, tx_cb);
}
...
{
    rb_add_element(&rx_buf, data);
}

void tx_cb(void)
{
    if (tx_buf.size > 0) {
        char data = (char)rb_get_element(&tx_buf);
        uart_write(UART_0, data);
    } else {
        uart_tx_end(UART_0);
    }
}
